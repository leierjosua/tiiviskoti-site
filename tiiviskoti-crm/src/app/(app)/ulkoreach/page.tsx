...
export default async function UlkoreachPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHead
        title="Ulkoreach — isännöinti"
        sub={campaign ? `Kylmä B2B-kampanja: ${campaign.name}` : 'Ei kampanjaa'}
        action={campaign ? (
      ...
      {campaign && (
        <p className="text-xs text-muted">
          Tila: <b>{active ? 'Aktiivinen' : campaign.status === 'paused' ? 'Tauolla' : 'Luonnos'}</b>.
          Hyväksytyt erät lähtevät automaattisesti (max {campaign.daily_cap}/vrk, klo 8–17), kun kampanja on aktiivinen
          ja mail.tiiviskoti.fi on verifioitu.
        </p>
      )}

      {stat.email < stat.total && (
        <div className="rounded-lg border border-warn/30 bg-warn/10 px-4 py-3 text-sm text-warn">
          <b>{stat.total - stat.email}</b> prospektilta puuttuu sähköposti — ne eivät lähde ennen kuin osoite on lisätty.
          {' '}<b>{stat.email}</b> on valmiina lähetykseen.
        </div>
      )}

      <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
        <Stat label="Prospektit" value={stat.total} />
      ...
      <Card>
        {campaign
          ...
          : <p className="text-sm text-muted">Kampanjaa ei löytynyt.</p>}
      </Card>
    </div>
  );
}

...
export default function FinalizeBooking() {
  ...
  // Contract offer (in summary step)
  ...
  // Templates matching this booking's service
  ...
  // Fetch PDF preview when template is selected (optionally with signature)
  const pickContractTemplate = useCallback((t: ContractTemplate) => {
    ...
    if (tiers.length === 1) {
      ...
      setSelectedContractTier(onlyTier);
    } else {
      setSelectedContractTier(null);
    }
  }, []);

  // Resolved unit + total contract pricing for current tier + device count.
  ...
  // Cleanup blob URL on unmount or template change
  useEffect(() => {
    return () => {
  ...
  // Build full line items list with proper quantity, line_type, and IDs
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // status update, line items, receipt email, satisfaction webhook, contract creation
      ...
      const { error } = await supabase.functions.invoke("finalize-booking", {
        body: {
      ...
      navigate(`/varaukset/${booking.booking_number}`);
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-text-muted mb-6">
        {customerName} · {serviceName} · {formatDate(booking.booking_date)}
      </p>
      ...
      {step === 0 && (
        <div className="space-y-4">
          <div className="bg-surface rounded-2xl border border-border p-6">
            <h2 className="font-semibold text-text-primary mb-4">Varauksen yhteenveto</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-text-muted text-xs uppercase tracking-wide mb-1">Päivämäärä & aika</p>
                <p className="font-medium text-text-primary">
                  {formatDate(booking.booking_date)} klo {booking.time_slot?.slice(0, 5)}
                </p>
              </div>
              <div>
                <p className="text-text-muted text-xs uppercase tracking-wide mb-1">Asentaja</p>
                <p className="font-medium text-text-primary">{installerName}</p>
              </div>
              <div>
              ...
              {(existingLineItems || []).length > 0 && (
                <div>
                  <p className="text-text-muted text-xs uppercase tracking-wide mb-1">Lisätyöt</p>
                  <p className="font-medium text-text-primary">
                    {(existingLineItems || []).map((li) => li.name).join(", ")}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              ...
              className="p-6 bg-surface rounded-2xl border-2 border-border hover:border-accent transition-all text-left group"
            >
              <div className="text-2xl mb-2">✅</div>
              <h3 className="font-semibold text-text-primary mb-1">Vakiokeikka</h3>
              <p className="text-sm text-text-muted">
                Varaus toteutui suunnitelman mukaan, ei muutoksia.
              </p>
            </button>
            ...
            >
              ...
              <h3 className="font-semibold text-text-primary mb-1">Muokkaa / lisää</h3>
              <p className="text-sm text-text-muted">
                Lisää palveluita tai ylimääräisiä veloituksia.
              </p>
            </button>
          </div>
        </div>
      ...
      {/* Step 1 — Modify (only for "modify" path) */}
      {step === 1 && (
      ...
      {step === 2 && (
        <div className="space-y-6">
          <div className="bg-surface rounded-2xl border border-border p-6">
            <h2 className="font-semibold text-text-primary mb-2">Maksutilanne</h2>
            <p className="text-sm text-text-muted mb-6">
              Lopullinen hinta: <span className="font-bold text-text-primary">{formatCents(finalPrice)}</span>
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button
              ...
              >
                <div className="text-2xl mb-2">💳</div>
                <h3 className="font-semibold text-text-primary mb-1">Maksettu</h3>
                <p className="text-sm text-text-muted">Asiakas on maksanut keikan.</p>
              </button>
              ...
              >
                <div className="text-2xl mb-2">⏳</div>
                <h3 className="font-semibold text-text-primary mb-1">Ei maksettu</h3>
                <p className="text-sm text-text-muted">Laskutetaan jälkikäteen.</p>
              </button>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface rounded-2xl border border-border p-6">
            ...
            <div className="space-y-2 text-sm">
              {initialized ? (
                // Modified booking — show full line item list
          ...
          {/* Protocol toggle — shown when protocol is completed */}
                    ...
                    <>
                      ...
                      <button
                        onClick={() => {
                          ...
                          // Re-generate PDF with signature stamped
...
function TemplateCard({
  ...
  return (
    ...
    >
      ...
      {savings > 0 && (
        <p className="text-sm text-accent-dark font-semibold mb-2">

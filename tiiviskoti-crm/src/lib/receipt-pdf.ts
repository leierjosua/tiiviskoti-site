...
/* Kuitti-PDF (Node). Portattu supabase/functions/_shared/generate-receipt-pdf.ts:stä
   CRM:n tk-datamalliin. Työn osuus = kiinteä 90 % kokonaishinnasta (sis. ALV),
...
export async function generateReceiptPdf(input: ReceiptInput): Promise<Uint8Array> {
  ...
  text('KUITTI', width - margin - bold.widthOfTextAtSize('KUITTI', 28), y - 20, { font: bold, size: 28 });
  y -= 62;

  // meta
  const metaR = width - margin;
  for (const [label, value] of [
    ['Kuittinumero:', `#${input.jobNumber}`],
    ['Tilauspvm:', fmtDate(input.createdAt)],
    ['Työn pvm:', fmtDate(input.workDate)],
  ]) {
  ...
  text('MAKSETTU', bx + (bw - bold.widthOfTextAtSize('MAKSETTU', 10)) / 2, y - bh + 17, { font: bold, size: 10, color: BRAND_GREEN });
  ...
  text('Kiitos kun valitsit TiivisKodin!', margin, y, { size: 10, color: GRAY });
  y -= 14;
  ...
  text('Y-tunnus 3652671-7', width - margin - font.widthOfTextAtSize('Y-tunnus 3652671-7', 9), 48, { size: 9, color: GRAY });

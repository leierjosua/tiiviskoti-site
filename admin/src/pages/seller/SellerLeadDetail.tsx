...
import { useSalesLead, useUpdateSalesLead, useLeadNotes, useCreateLeadNote, useLeadEvents } from "@/hooks/sales/useSalesLeads";
...
export default function SellerLeadDetail() {
  ...
  async function handleStatusChange(status: LeadStatus) {
    await updateLead.mutateAsync({ id: lead!.id, status, last_contact_at: new Date().toISOString() });
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate("/myyja/kylmasoitot")} className="p-1.5 rounded-lg hover:bg-muted transition-colors">
          <ArrowLeft className="w-4 h-4" />
        </button>
        <div className="flex-1">
          <h1 className="text-lg font-bold">{lead.name || "Nimetön liidi"}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-surface border border-border rounded-2xl p-4">
            <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-3">Muistiinpanot</h3>
            <NoteTimeline
              notes={notes}
              events={events}
